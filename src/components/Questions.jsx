...
function Questions() {
  ...
  const handleDown = (e) => {
    ...
    if (Faqs.id == "faqs-1") {
      let faqs = e.currentTarget
      IsFaqsOne ? faqs.className = "rounded-md w-11/12 mx-auto flex flex-col items-center overflow-y-hidden bg-dark h-48" : faqs.className = 'rounded-md w-11/12 mx-auto flex flex-col items-center justify-between overflow-y-hidden bg-dark h-12'

      IsFaqsOne ? setIsFaqsOne(false) : setIsFaqsOne(true)
    }
    if (Faqs.id == "faqs-2") {
      let faqs = e.currentTarget
      IsFaqsTwo ? faqs.className = "rounded-md w-11/12 mx-auto flex flex-col items-center justify-between overflow-y-hidden bg-dark h-48" : faqs.className = 'rounded-md w-11/12 mx-auto flex flex-col items-center justify-between overflow-y-hidden bg-dark h-12 '
      IsFaqsTwo ? setIsFaqsTwo(false) : setIsFaqsTwo(true)
    }
    if (Faqs.id == "faqs-3") {
      let faqs = e.currentTarget
      IsFaqsThree ? faqs.className = "rounded-md w-11/12 mx-auto flex flex-col items-center justify-between overflow-y-hidden bg-dark h-48" : faqs.className = 'rounded-md w-11/12 mx-auto flex flex-col items-center justify-between overflow-y-hidden bg-dark h-12 '
